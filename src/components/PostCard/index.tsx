...
// ============================================================================
// Mock Data & View Helpers (PRレビュー・デバッグ用)
// ============================================================================

/**
 * 動作確認用のダミーデータです。
 * page.tsxコンフリクトを避けるため、レビュアーが自由にimportして利用できるようにしています。
 */
export const dummyPostCardProps: PostCardProps = {
  title: "記事タイトル：いろはにほへとちりぬるをわかよたれそつねならむうゐのおくやまけふこえてあさきゆめみしゑひもせす",
  author: "山田 太郎 いろはにほへとちりぬるをわかよたれそつねならむうゐのおくやまけふこえてあさきゆめみしゑひもせす",
  category: "テックいろはにほへとちりぬるをわかよたれそつねならむうゐのおくやまけふこえてあさきゆめみしゑひもせす",
  thumbnailUrl: "/sample1.jpg",
  content:
    "この記事では〜について解説します。いろはにほへとちりぬるをわかよたれそつねならむうゐのおくやまけふこえてあさきゆめみしゑひもせす",
  createdAt: new Date().toISOString(),
};

/**
 * 開発プレビュー時に、ローカルの page.tsx 等で1行呼び出すだけで
 * 8個分のカードをグリッド表示させるためのテスト用ラッパーコンポーネントです。
 * ※本番のUI構築には使用しません。
 */
export const DummyPostCards = (props: PostCardProps) => {
  return (
    <div className={styles.grid}>
      {Array.from({ length: 8 }).map((_, i) => {
        const baseDate = new Date(props.createdAt);
        baseDate.setDate(baseDate.getDate() - i * i); // サンプルとして i*i 日ずつ過去にズラす処理

        return (
          <PostCard
            key={i}
            {...props}
            thumbnailUrl={`/sample${(i % 5) + 1}.jpg`} // 表示崩れ確認のため複数のサンプル画像を使い回し
